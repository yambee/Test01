...
for (i=0, j={}; i <= st.length; i++) {	
	if (isStopCh(st[i]) || i === st.length) { // если st[i] стопсимвол или мы достигли конца строки st, то добавляем строку w в её текущем виде как атрибут объекта j и плюсуем ему счетчик (если значение атрибута undefined, значит слово новое, ставим 1). В случае если стоп-символы идут один за другим, например пробел после точки, то в объект j будет добавлен пустой атрибут j[""] (соотв-щее кол-во раз). Он будет удален после цикла.
