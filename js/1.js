...
for (i=0, j={}; i <= st.length; i++) {	
	if (isStopCh(st[i]) || i === st.length) { // если st[i] стопсимвол или мы достигли конца строки st, 
		if (j[w]) {j[w]++} else {j[w]=1}; // то добавляем строку w в её текущем виде как атрибут объекта j и плюсуем ему счетчик (если значение атрибута j[w] undefined, значит слово новое, ставим аотрибуту значение 1). В случае если стоп-символы идут один за другим, например пробел после точки, то в объект j будет добавлен пустой атрибут j[""] (соотв-щее кол-во раз). Он (пустой атрибут) будет удален после цикла.
		w = ""
	} else {
		w += st[i]
	}
