...
for (i=0, j={}; i<st.length; i++) {
	if (isStopCh(st[i])) { // если st[i] стопсимвол, то добавляем строку w в объект и плюсуем ему счетчик (если слово новое, ставим 1). В случае если стоп-символы идут один за другим, например пробел после точки, то в объект j будет добавлен пустой элемент w (соотв кол-во раз). Это пока не проблема.
